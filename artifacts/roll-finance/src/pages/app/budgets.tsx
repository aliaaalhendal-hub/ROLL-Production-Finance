  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="flex justify-between items-end border-b border-border pb-6">
        <div>
          <h1 className="font-serif text-4xl mb-2">Department Budgets</h1>
          <p className="text-muted-foreground font-light">Monitor allocations, commitments, and actuals across all departments.</p>
        </div>
        ...
        <Dialog open={isCreateOpen} onOpenChange={(open) => { setIsCreateOpen(open); if(!open) { setEditingId(null); form.reset(); } }}>
          ...
          <DialogContent className="sm:max-w-md bg-card border border-border rounded-none">
            <DialogHeader>
              <DialogTitle className="font-serif text-2xl">{editingId ? 'Edit' : 'Add'} Department Budget</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pt-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="uppercase text-xs tracking-widest text-muted-foreground">Department Name</FormLabel>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="uppercase text-xs tracking-widest text-muted-foreground">Allocated Budget ($)</FormLabel>
                      <FormControl>
                ...
                <button type="submit" disabled={createBudget.isPending || updateBudget.isPending} className="w-full bg-primary text-primary-foreground p-4 text-sm font-semibold uppercase tracking-widest hover:bg-primary/90 transition-colors flex justify-center items-center">
                  {(createBudget.isPending || updateBudget.isPending) ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Save Budget'}
                </button>
              </form>
            </Form>
          </DialogContent>
            ...
            <div key={budget.id} className="p-6 border border-border bg-card group relative overflow-hidden">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <div className="flex gap-4 mt-2 text-xs font-semibold uppercase tracking-widest">
                    <span className="text-muted-foreground">Allocated: <span className="text-foreground">${budget.allocated.toLocaleString()}</span></span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-4 gap-4 mb-6">
                <div className="p-4 bg-background border border-border">
                  <div className="text-xs uppercase tracking-widest text-muted-foreground mb-1">Paid</div>
                  <div className="font-serif text-xl">${budget.paid.toLocaleString()}</div>
                </div>
                <div className="p-4 bg-background border border-border">
                  <div className="text-xs uppercase tracking-widest text-muted-foreground mb-1">Committed</div>
                  <div className="font-serif text-xl">${budget.committed.toLocaleString()}</div>
                </div>
                <div className="p-4 bg-background border border-border">
                  <div className="text-xs uppercase tracking-widest text-muted-foreground mb-1">Remaining</div>
                  <div className="font-serif text-xl text-primary">${budget.remaining.toLocaleString()}</div>
                </div>
                <div className="p-4 bg-background border border-border">
                  <div className="text-xs uppercase tracking-widest text-muted-foreground mb-1">Forecast</div>
